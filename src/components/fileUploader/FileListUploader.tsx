...
export default function FileListUploader({ fileList, onUpload, id, className }: FileListUploaderProps) {
  const { current: defaultImage } = useRef<string>("/images/image_default_image.png");
    ...
    (e: ChangeEvent) => {
      ...
      if (!fileList.length) return;
        ...
        if (uploadedFileList.length !== fileList.length) {
          const exceptedFileNameList = preservedFileList.filter((file) => !preservedFileList.includes(file)).map((file) => file.name);
          window.alert(`아래 이미지들은 허용 사이즈를 초과해 제외되었습니다.\n${exceptedFileNameList.join(`\n`)}`);
        }
  ...
  return (
    <div className="file-list-container">
      <label className={clsx("file-list-uploader", className)} htmlFor={id || "file-list-uploader-input"} title="이미지 업로드">
        <Image src={defaultImage} alt="" width={56} height={56} quality={100} priority />
        <input ref={inputRef} type="file" id={id || "file-list-uploader-input"} onChange={changeHandler} accept="image/*" multiple />
      </label>
      {isUploaded &&
        fileList.map((file, idx) => (
          <article key={file?.name || file} className="file-list-item">
            <Image src={imageUrlList[idx] || defaultImage} alt="" width={56} height={56} quality={100} priority />
          </article>
        ))}
    </div>
  );
